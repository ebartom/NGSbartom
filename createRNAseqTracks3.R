...
stranded <- sub('--stranded=', '', args[grep('--stranded=',args)])

if (identical(multiMap,character(0))){
    multiMap <- 0
}

if (identical(stranded,character(0))){
    stranded <- 1
}
...
bam2bw <- function(BF,organism){
    ...
    if (stranded == 1) {
       ...
       strand(bd) <- ifelse(strand(bd) == '+', '-', '+')
    	...
    	## minus coverage
    	cat("getting coverage for minus strand\n")
      ...
      plusminus <- coverage(bd)
      plusminus.rpm <- plusminus*(1e6/length(bd)) ## normalizing to the library size
      seqlengths(plusminus.rpm) <- seqlengths(organism)[names(plusminus.rpm)]
      ## export rpm to bigWig
      plusminus.outfile <- sub(".bam", "", BF)
      plusminus.outfile <- paste(plusminus.outfile,multiString, ".bw", sep="")
      cat(paste("exporting to bigwig", plusminus.outfile, "\n", sep="\t"))
      export.bw(plusminus.rpm, plusminus.outfile)
      cat("export complete:", BF, sep="\n")
   }
}
