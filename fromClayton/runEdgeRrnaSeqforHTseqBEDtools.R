...
rpkmFile <-sub('--rpkmFile=', '', args[grep('--rpkmFile=', args)])
...
print(rpkmFile)
rpkm <- read.table(file=rpkmFile,header=TRUE,sep="\t",row.names=1)
print(countFile)
counts <- read.delim(file=countFile,header=TRUE,sep="\t",row.names=1)
head(counts)
dim(counts)
head(rpkm)
dim(rpkm)
...
allData<-data.frame(counts[,1:length(colnames(counts))-1])	


print (runMDS)
if (runMDS==1){
   print("Generating MDS plot.")
   newnames <- counts[,1:length(colnames(counts))-1]
...
runEdgeR <- function(data,rpkm,counts,comparison){
#	 print(head(data))
#	 print(comparison)
	 group1 <- c()
	 group2 <- c()
...
#	 print("making annotated dataframe\n")
	 ...
	 #print(dim(counts[foo,]))
	 print(head(rpkm))
         print(dim(rpkm))
	 print(dim(rpkm[foo,]))
    	 df <- cbind(counts[foo,],rpkm[foo,],lrt$table,adj.p=p.adjust(lrt$table$PValue,method="BH"))
...
#   print(rownames(comparisons))
   ...
   for(c in 1:length(rownames(comparisons))){
    	...
    	comp <-runEdgeR(data=allData,rpkm,counts,comparison)
