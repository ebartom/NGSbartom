...
#    names(geneList) <- GOterms   
    ...
    print("Testing")
#    print(head(x))
#    exampleGO.ID <- x[2,"GO.ID"]
#    print(exampleGO.ID)
#    exampleGeneList <- toString(genesInTerm(GOData,exampleGO.ID))
#    exampleGeneList <- gsub("\\\\","",exampleGeneList)
#    exampleGeneList <- gsub(" ","",exampleGeneList)
#    exampleGeneList <- gsub("\"","",exampleGeneList)
...
#    print(exampleGeneList)
                                        #    x$geneSetInTerm <- toString(genesInTerm(GOData,x$GO.ID))
    allGO = genesInTerm(GOData)
    geneSetsforGOterms = lapply(allGO,function(x) x[x %in% geneList] )
    x$geneSetInTerm <- geneSetsforGOterms[[x$GO.ID]]
    x$geneSetInTerm <- gsub("\\\\","",x$geneSetInTerm)
    x$geneSetInTerm <- gsub(" ","",x$geneSetInTerm)
    x$geneSetInTerm <- gsub("\"","",x$geneSetInTerm)
    x$geneSetInTerm <- gsub("\n","",x$geneSetInTerm)
    print("Did it work?")
    print(head(x$GO.ID))
    print(head(x$geneSetInTerm))
    print(fname)
