...
GetOptions('samplesheet|ss=s' => \$sampleSheet,
	   ...
	   'extsize|es=i' => \$extsize,
	   'nomodel|nm=i' => \$nomodel
    ) ;
...
# Define references.
my (%bowtieIndex,%txIndex,%txdbfile,%bwaIndex,%gff,%exonbed,%rsemTx,%genebed);
...
#	    print SH "samtools flagstat $outputDirectory\/$project\/bam\/$sample.bam > $outputDirectory\/$project\/bam\/$sample.flagstats.txt\n";
		...
		if ($stranded == 1){
		    ...
		    print SH "Rscript $NGSbartom/tools/createRNAseqTracks3.R --assembly=$assembly --bamDir=$outputDirectory\/$project\/bam\/ --sample=$sample --multiMap=$multiMap --stranded=$stranded\n";
		} else {
		    my $assembly = $reference{$sample};
		    if ($assembly eq "hg38.mp"){$assembly="hg38";}
		    # The multi mapping argument is used here, because these are mapped with tophat.
#		    print SH "Rscript $NGSbartom/tools/createChIPtracks2.R --assembly=$assembly --bamDir=$outputDirectory\/$project\/bam\/ --sample=$sample --extLen=0 --multiMap=$multiMap\n";
		    print SH "Rscript $NGSbartom/tools/createRNAseqTracks3.R --assembly=$assembly --bamDir=$outputDirectory\/$project\/bam\/ --sample=$sample --multiMap=$multiMap --stranded=$stranded\n";
		}
		print SH "date\n\n";
...
#	    print STDERR "$sample\tREF:$reference{$sample}\tINDEX:$bowtieIndex{$reference{$sample}}\n";
		...
		if ($makeTracks == 1){
		    ...
		    my $assembly = $reference{$sample};
		    if ($assembly eq "hg38.mp"){ $assembly = "hg38";}
		    print SH "Rscript $NGSbartom/tools/createChIPtracks.R --assembly=$assembly --bamDir=$bamDirectory --sample=$sample --extLen=$extsize\n";
		    print SH "date\n\n";
		    print SH "mkdir $outputDirectory\/$project\/tracks\n";
		    if ($uploadASHtracks == 1){
			print SH "# Move tracks into Shilatifard directory structure\n";
...
if ($buildEdgeR ==1) {
	...
	if ($type eq "RNA"){
		...
		if ($makeTracks == 1){
			...
			my $assembly = $reference{$sample};
			if ($assembly eq "hg38.mp"){$assembly="hg38";}
			print SH "Rscript $NGSbartom/tools/createRNAseqTracks3.R --assembly=$assembly --bamDir=$bamDirectory\/ --sample=$sample --stranded=$stranded  --multiMap=$multiMap\n";
			print SH "date\n\n";
			print SH "mkdir $outputDirectory\/$project\/tracks\n";
			print SH "\n# Make Headers for UCSC genome browser.\n";
			print SH "echo \"track type=bigWig name=$sample.plus.bw description=$sample.plus.rpm graphtype=bar maxHeightPixels=128:60:11 visibility=full color=255,0,0 itemRGB=on autoScale=on bigDataUrl=https://s3-us-west-2.amazonaws.com/$s3path/$scientist.$project/$sample.plus.bw\" | cat > $outputDirectory\/$project\/tracks\/$sample.plus.bw.header.txt\n";
		...
		if ($assembly eq "hg38.mp"){$assembly="hg38";}
		print SH "Rscript $NGSbartom/tools/createRNAcounts.R --assembly=$assembly --bamDir=$bamDirectory --numCores=$numProcessors --txdbfile=$txdbfile{$reference{$project}}\n";
...
if (($buildPeakCaller ==1) && ($type eq "chipseq")){
	...
	if ($chipDescription ne ""){
	    ...
	    if (($broadPeakCheck ne "") && ($peakcaller eq "")){
		...
		if (($_ !~ /^IP/)&& ($_ !~ /^ChIP/)){
		    ...
		    if (($peakType eq "narrow") || ($peakcaller eq "macs2")) {
			...
			if ($peakType eq "narrow") {
			    ...
			    print SH "#MSUB -N $ip\_BroadPeaks\n";
			}
			print SH "#MSUB -l nodes=1:ppn=$numProcessors\n";
			#print SH "module load R/3.2.2\n";
			if ($peakcaller eq "") {
			...
			elsif ($peakcaller eq "macs2") {
			    print SH "export PATH=\$PATH:$NGSbartom/tools\n";
			    print SH "module load MACS2\n";
			    print SH "\n# Call $peakType peaks for ip file $ip, input $input using macs2\n";
			    print SH "\n# Make fold enrichment track for file $ip, input $input using macs2\n";
			    print SH "cd $outputDirectory\/$project\/peaks\n";
			    if ($peakType eq "narrow") {
				if ($nomodel) {
				    print SH "macs2 callpeak --nomodel --extsize $extsize -f BAM -t $bamDirectory\/$ip.bam -c $bamDirectory\/$input.bam -g $ref -B --SPMR -n $ip.macsPeaks >& $ip.macs2.log\n";
				}
				else {
				    print SH "macs2 callpeak -f BAM -t $bamDirectory\/$ip.bam -c $bamDirectory\/$input.bam -g $ref -B --SPMR -n $ip.macsPeaks >& $ip.macs2.log\n";
				}
				print SH "awk '{printf \"%s\\t%d\\t%d\\t%s\\t%d\\n\",\$1,\$2,\$3,\$4,\$5}' < $ip.macsPeaks_peaks.narrowPeak > $ip.macsPeaks_peaks.bed\n";
			    }
			    elsif ($peakType eq "broad") {
				if ($nomodel) {
				    print SH "macs2 callpeak --nomodel --extsize $extsize --broad -f BAM -t $bamDirectory\/$ip.bam -c $bamDirectory\/$input.bam -g $ref -B --SPMR -n $ip.macsPeaks >& $ip.macs2.log\n";
				}
				else {
				     print SH "macs2 callpeak --broad -f BAM -t $bamDirectory\/$ip.bam -c $bamDirectory\/$input.bam -g $ref -B --SPMR -n $ip.macsPeaks >& $ip.macs2.log\n";
				}
				print SH "awk '{printf \"%s\\t%d\\t%d\\t%s\\t%d\\n\",\$1,\$2,\$3,\$4,\$5}' < $ip.macsPeaks_peaks.broadPeak > $ip.macsPeaks_peaks.bed\n";
			    }
			    print SH "macs2 bdgcmp -t $ip.macsPeaks_treat_pileup.bdg -c $ip.macsPeaks_control_lambda.bdg -o $ip\_FE.bdg -m FE\n";
			    print SH "LC_COLLATE=C sort -k1,1 -k2,2n $ip\_FE.bdg > $ip\_FE.sorted.bdg\n";
			    print SH "bedGraphToBigWig $ip\_FE.sorted.bdg $NGSbartom/anno/chromSizes/$reference{$project}\.chrom.sizes $outputDirectory\/$project\/tracks\/$ip\_FE.bw\n";
			    print SH "rm $ip\_FE.bdg $ip\_FE.sorted.bdg $ip.macsPeaks_treat_pileup.bdg $ip.macsPeaks_control_lambda.bdg\n";
			    print SH "date\n";
			}
			if ($makeTracks){
...
#			print SH "bedtools coverage -a $outputDirectory\/$project\/peaks\/$ip.macsPeaks.bed -b $outputDirectory\/$project\/bam\/$ip.bam >  $outputDirectory\/$project\/peaks\/$ip.macsPeaks.cov.txt\n"; (Carlos: wrong format error)
			print SH "bedtools coverage -abam $outputDirectory\/$project\/bam\/$ip.bam -b $outputDirectory\/$project\/peaks\/$ip.macsPeaks.bed >  $outputDirectory\/$project\/peaks\/$ip.macsPeaks.cov.txt\n";
		    ...
		    } elsif (($peakType eq "broad") && ($peakcaller eq "")){
			print BSH "module load bedtools/2.17.0\n";
			...
			print BSH "SICER.sh $bamDirectory $ip.bed $input.bed $outputDirectory\/$project\/peaks\/$ip.sicer $reference{$project} 1 200 $extsize 0.8 600 1e-8 >& $outputDirectory\/$project\/peaks\/$ip.sicer.log\n";
